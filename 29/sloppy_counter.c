...
static void init(counter_t *c, int threshold) {
  ...
  c->global = 0;
  Pthread_mutex_init(&c->glock, NULL); // init global lock
  for (int i = 0; i < NUMCPUS; i++) {
    c->local[i] = 0;
    Pthread_mutex_init(&c->llock[i], NULL); // init local lock
  }
}
...
int main() {
  for (int i = 0; i < 11; i++) {
    double threshold = pow(2.0, (double)i); // increment threshold * 2
